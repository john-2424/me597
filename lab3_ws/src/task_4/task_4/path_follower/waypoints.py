...
class WayPoints():
    ...
    def bot_on_path(self, bot_pose, tol=0.2):
        ...
        """
        bot_pose = np.array([bot_pose.pose.position.x, bot_pose.pose.position.y])

        pts1 = self.waypoints[:-1]   # start of segments
        pts2 = self.waypoints[1:]    # end of segments
        seg_vec = pts2 - pts1   # vector along each segment
        pt_vec = bot_pose - pts1 # vector from start to robot

        # Project pt_vec onto seg_vec to find closest point parameter t
        seg_len2 = np.sum(seg_vec**2, axis=1)
        t = np.sum(pt_vec * seg_vec, axis=1) / seg_len2
        t = np.clip(t, 0.0, 1.0)  # constrain to segment

        # Closest points on each segment
        closest = pts1 + t[:, None] * seg_vec

        # Distances from robot to those closest points
        dists = np.linalg.norm(closest - bot_pose, axis=1)

        # Within tolerance?
        # print(np.any(dists <= tol))
        return np.any(dists <= tol)

    def bot_reached(self, bot_pose, tol=0.2):
        """
        ...
        target = self.waypoints[self.last_idx]
        dist = np.linalg.norm(bot_pose - target)
    ...
    def _extend_collinear_segment(self, start_idx, tol=1e-6):
        ...
        rel = self.waypoints[i+1:] - p0                    # shape (N-i-1, 2)
    ...
    def choose_next(self, bot_pose):
        # Get distances from each waypoint to the bot pose
        dx = self.waypoints[:,0] - bot_pose.pose.position.x
        dy = self.waypoints[:,1] - bot_pose.pose.position.y
        distances = np.sqrt(dx**2 + dy**2)

        # Mask to get all the waypoints after the last index
        mask = np.arange(len(self.waypoints)) > self.last_idx
        future_distances = np.where(mask, distances, np.inf)

        # Get the index with the minimum distance
        next_idx = np.argmin(future_distances)
